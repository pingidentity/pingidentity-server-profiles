...
if [[ ! -z "${OPERATIONAL_MODE}" && "${OPERATIONAL_MODE}" = "CLUSTERED_ENGINE" ]]; then
    ...
    keypairid=$( curl -v -k -u Administrator:2FederateM0re -H "X-Xsrf-Header: PingAccess" https://${pahost}:9000/pa-admin-api/v3/httpsListeners | jq '.items[] | select(.name=="CONFIG QUERY") | .keyPairId' )
    echo "KeyPairId:"${keypairid}

    echo "Retrieving the Key Pair alias..."
    kpalias=$( curl -v -k -u Administrator:2FederateM0re -H "X-Xsrf-Header: PingAccess" https://${pahost}:9000/pa-admin-api/v3/keyPairs | jq '.items[] | select(.id=='${keypairid}') | .alias' )
    echo "Key Pair Alias:"${kpalias}

    echo "Retrieving Engine Certificate ID..."
    certid=$( curl -v -k -u Administrator:2FederateM0re -H "X-Xsrf-Header: PingAccess" https://${pahost}:9000/pa-admin-api/v3/engines/certificates| jq '.items[] | select(.alias=='${kpalias}' and .keyPair==true) | .id' )
    ...
    curl -v -k -X POST -u Administrator:2FederateM0re -H "X-Xsrf-Header: PingAccess" https://${pahost}:9000/pa-admin-api/v3/engines/${engineid}/config -o engine-config.zip
